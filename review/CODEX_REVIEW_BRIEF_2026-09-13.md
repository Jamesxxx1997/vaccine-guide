...
- 已知判斷點請 Codex 挑戰：(1) Merckx Table 2 三個「整體差值」格（ⴚ25.5 等）因負號字元 `ⴚ` 在 pdfplumber 只得 (cid:3) 無法定位，改引 p.4–5 同值內文句（ref note 有註明；CrI 端點順序因此是 17.0–33.4 而非表格的 33.4→17.0）；(2) BinaxNOW Card 2 搭 DIGIVAL 判讀儀歸入 DIA 表（舊版標 RIDT）；(3) 數字守門迫使標籤性數字改寫：`Table 2`→表二、`IDSA 建議 7/10/11`→「IDSA 指引建議（門診／住院）」、`510(k)`→原廠上市前審查文件、`Sofia 2`→Sofia Ⅱ、`FilmArray RP2`→新一代呼吸道病原套組——頁面上 IDSA 建議編號消失，若要保留需補含編號的引句；(4) Vstrip 表只有一列（來源裡沒有其他台灣純肉眼 RIDT 機型）。
