...
## C. 檢驗判讀面板重組（2026-09-17）：以診斷工具×次族群為單位＋句中引註
- 使用者要求：pooled estimate＋95% CI 以「診斷工具（或工具＋次族群）」為單位呈現，不以論文篇為單位；一句話裡不同來源支撐的子句，各自在子句後標 [n]（像寫論文），不堆在句尾。
- 資料格式新增（`tools/build_clinical.py` docstring）：整理句／`tags.row` 格內可寫 `[[n]]`（n＝該 item 第 n 個 ref）→ 渲染成緊接子句的 [n]，未在句中引用的 ref 自動補在句尾或「來源」欄；同 `tags.topic` 的 item 都帶 `tags.row`（{欄名:格文字}）時渲染成估計值表（`clinical.js` `estimateTable`，一列＝一 item，每格自帶 [n]，編號依格內首次出現左→右）。守門：row 格內數字同受「必須出現在引句」檢查；`[[n]]` 超出 refs 數即拒絕；`numbers_ok` 掃描前先剝除 `[[n]]`。
- `review/clinical.src.d/flu/test.json`：41 → 54 items（113 claims）。7 張表：RIDT／DIA／rapid NAAT 統合估計（Merckx 2017 Table 2 整體、兒童、成人、兒童−成人差值、POC／非 POC、廠商贊助；IDSA 2018 轉引；CDC 範圍）、三類互比（差值＋納入研究數＋特異度＋兒童 vs 成人）、台灣機型 RIDT／DIA／分子（Vstrip、BD Veritor、BinaxNOW＋DIGIVAL、Sofia Ⅱ 一致性〔非對 PCR〕、疾管署 Sofia 本土研究、cobas Liat、ID NOW、Xpert Xpress、FilmArray）。4 張議題卡（採檢時機、檢體種類與下呼吸道檢體、門診／住院首選檢驗與快篩定位、快篩陰性要不要給藥）每機關一行、子句各自 [n]。
- 其他 7 個分片共 77 個 item 插入句中引註（qa.json 全單 ref 未動）。另補 3 個原本無原句的子句：special `flu-sp-asthma-relenza-warning`（S110 p.4）、vaccine `flu-vx-still-get-flu-after-vax`（S129 p.1，「病毒株」改「流感病毒」以貼原文）、vaccine-premed `flu-vx-anaphylaxis-epi-route-site`（S152 p.4，quote 截到 "…epinephrine"，因 pdftotext 把 "administration." 切到另一 block）。
- 已知判斷點請 Codex 挑戰：(1) Merckx Table 2 三個「整體差值」格（ⴚ25.5 等）因負號字元 `ⴚ` 在 pdfplumber 只得 (cid:3) 無法定位，改引 p.4–5 同值內文句（ref note 有註明；CrI 端點順序因此是 17.0–33.4 而非表格的 33.4→17.0）；(2) BinaxNOW Card 2 搭 DIGIVAL 判讀儀歸入 DIA 表（舊版標 RIDT）；(3) 數字守門迫使標籤性數字改寫：`Table 2`→表二、`IDSA 建議 7/10/11` 一度改成文字描述，使用者要求保留編號→已把三條 quote 改成含原文編號的整句（「7. Clinicians should collect…」「10. …」「11. …」），整理句寫「IDSA 建議 7／10／11」、`510(k)`→原廠上市前審查文件、`Sofia 2`→Sofia Ⅱ、`FilmArray RP2`→新一代呼吸道病原套組；(4) Vstrip 表只有一列（來源裡沒有其他台灣純肉眼 RIDT 機型）。
- 獨立驗證（fresh-context，做的人不自驗）：Merckx Table 2 19 列四欄逐格對 pdftotext 無放錯欄／串列；台灣機型 8 款每個 PPA／NPA／n／CI 對 IFU／510(k) 逐字相符；11 個 p.8 region claim 裁圖＋`pdftotext -bbox-layout` 座標比對 offset 0（verifier 曾因直接用 pdfplumber 未減 bbox 原點誤判 +9pt 位移，自行更正）；句中引註抽 15 item＋3 議題卡逐子句對位全對；舊 41 item 每條 (source,page,quote) 在新檔全數出現；`node tools/test_clinical.cjs`：7 estimate tables（149 numeric cells each with own mark）、358 items、602 anchored refs。
- 請 Codex 檢查：(a) 用 §6.2 數值法對 `cl:flu:flu-test-*` 113 個 claim 全量核對；(b) 其餘 62 個未抽樣的句中引註 item 是否有 `[[n]]` 標到不支撐該子句的 ref（機械測試抓不到語意歸屬錯誤）；(c) 表格欄名「95% CrI」是否與每列實際區間型態一致（IDSA／CDC 列沒有 CI，格內已寫「無 CI」或範圍）。
