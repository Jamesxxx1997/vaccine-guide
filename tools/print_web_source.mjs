...
// 用法：node tools/print_web_source.mjs <url> <out.pdf> [--expand "全部展開"] [--wait 2500] [--ua "Mozilla/5.0 …"]
//   --expand  先點擊頁面上文字等於該字串的元素（a/button/div/span/li/label；例：疾管署 Q&A 與 TFDA 仿單頁的「全部展開」，否則手風琴內容不會印出）
...
const ua = opt('--ua', null);   // 例：mcp.fda.gov.tw 的 WAF 會擋 headless Chrome 預設指紋，帶一般瀏覽器 UA 即可（不改寫 DOM）
...
if (ua) { await send('Network.enable'); await send('Network.setUserAgentOverride', { userAgent: ua }); }
...
let expanded = null;
if (expandText) {
  expanded = await evaluate(`(() => { const els=[...document.querySelectorAll('a,button,div,span,li,label')].filter(e=>e.textContent.trim()===${JSON.stringify(expandText)}); els.forEach(e=>e.click()); return els.length; })()`);
