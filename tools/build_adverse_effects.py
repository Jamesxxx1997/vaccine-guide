...
def build_notes(table, words, width):
    ...
    for i, r in enumerate(table['rows']):
        ...
        claims[cid] = dict(note=table.get('note', ''), items=[dict(source=table['source'], page=table['page'], glyphRows=True,
                           quotes=[r['quote']], label=f'{table["product"]} · {r.get("label", "說明")}')])
...
def main():
    ...
    js = json.dumps(dict(tables=out_tables, generatedFrom='review/adverse-effects.src.json'), ensure_ascii=False).replace('</', '<\\/')
