...
module.exports=function setup(opts={}){const {beforeScripts}=opts;
  ...
  const dom=new JSDOM(fs.readFileSync(path.join(root,'index.html'),'utf8'),{
    url:(opts&&opts.url)||'http://localhost:8899/index.html',runScripts:'outside-only',pretendToBeVisual:true,virtualConsole:logs});
