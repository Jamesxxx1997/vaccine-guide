...
def build():
    ...
    # 逐字 glyph 框（chars）只有舊摘錄系統在瀏覽器端即時算框才需要；pagesOnly:"claims" 的來源全部走預先算好的 rects，
    # 定位完成後不輸出 chars（2026-09-16：chars 佔 reference-pages.js 45.8 MB 中的 36.3 MB，拖慢 CI 測試與使用者下載）
    stripped = 0
    for key, doc in sources.items():
        if isinstance(doc, dict) and doc.get('pagesOnly') == 'claims':
            for pg in doc.get('pages', []):
                if pg.pop('chars', None) is not None:
                    stripped += 1
    print(f'stripped chars from {stripped} claims-only pages')
