...
用法：uv run --python 3.12 python tools/sweep_allergens.py [--source S33]（--source 只重掃該來源，寫 review/allergen_sweep.d/<來源>.json 後重建合併報告）
"""
...
SYN = {
    'egg': [r'雞胚', r'雞蛋', r'蛋類', r'卵白', r'卵蛋白', r'(?<![蛋])蛋(?![白])', r'(?<!卵)卵(?!巢|磷脂)', r'禽', r'ovalbumin', r'\begg', r'chick', r'embryo', r'\bhen\b', r'\bhens\b', r'avian'],
    ...
    'peg_polysorbate': [r'聚山梨', r'吐溫', r'聚乙二醇', r'polysorbate', r'\btween\b', r'\bpeg\b', r'polyethylene\s*glycol', r'macrogol', r'polyoxyethylene', r'聚氧乙烯'],
...
SWEEP_DIR = ROOT / 'review/allergen_sweep.d'   # 每個來源一檔（多 agent 並行不打架）；combined json/md 由目錄重建

def render(report):
    md = ['# 「未載明」全文掃描報告', '每格列出命中詞、頁碼、前後文；判讀後請把結果寫回分片（有／無＋quote、related，或 sweep.dismissed）。掃描紀錄以 review/allergen_sweep.d/<來源>.json 為準。', '']
    total_cells = total_hits = 0
    for sid, r in report.items():
        md.append(f'## {sid} {r["product"]}（{Path(r["pdf"]).name}，{r["pages"]} 頁；掃描 {r.get("date","")}）')
        for key, c in r['cells'].items():
            total_cells += 1; total_hits += len(c['hits'])
            tag = '（分片已判為假陽性：' + c['dismissed'] + '）' if c.get('dismissed') else ''
            md.append(f'### {key}：{len(c["hits"])} 命中{tag}')
            for h in c['hits']:
                md.append(f'- p.{h["page"]}「{h["term"]}」…{h["context"]}…')
        md.append('')
    return md, total_cells, total_hits
...
def main():
    ...
    for frag in sorted(SRC_DIR.glob('*.json')):
        ...
        if sum(len(t[2]) for t in texts) < 200:
            print(f'! {frag.stem} {d["product"]}：PDF 無文字層（掃描檔），無法掃描'); continue
        entry = dict(product=d['product'], source=d['source'], pdf=str(pdf.relative_to(ROOT)), pages=len(texts), date=today, cells={})
        dismissed = (d.get('sweep') or {}).get('dismissed', {})
        for key in pending:
            hs = hits_for(texts, SYN[key])
            entry['cells'][key] = dict(patterns=SYN[key], hits=hs, dismissed=dismissed.get(key))
        report[frag.stem] = entry
        (SWEEP_DIR / f'{frag.stem}.json').write_text(json.dumps(entry, ensure_ascii=False, indent=1), encoding='utf-8')
    report = dict(sorted(report.items(), key=lambda kv: (len(kv[0]), kv[0])))
    md, total_cells, total_hits = render(report)
    ...
    print(f'{total_cells} 個未載明格子，{total_hits} 個候選命中 → review/allergen_sweep.md（{len(report)} 個來源）')
